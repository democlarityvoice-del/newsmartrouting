...
/* ===== Intelli Routing — Portal overlay (jQuery, Clarity orange) ===== */
;(function(){
  // brand
  var ORANGE     = '#e57027';
  ...
  function ensureStyle(){
    ...
    var css = ''
      + '#cv-intelli-root{position:fixed;inset:0;z-index:999999;display:none;}'
      + '#cv-intelli-root .cv-back{position:absolute;inset:0;background:rgba(0,0,0,.25);}'
      + '#cv-intelli-root .cv-panel{position:absolute;top:6%;left:50%;transform:translateX(-50%);'
      + 'width:960px;max-width:90vw;background:#fff;border-radius:12px;box-shadow:0 20px 80px rgba(0,0,0,.25);font:14px/1.4 system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;}'
      ...
      + '#cv-intelli-root .cv-b{padding:16px;max-height:72vh;overflow:auto;}'
      /* UI styles */
  ...
  // open overlay + mount app
  function openOverlay(){
    var $root = ensureRoot(); $root.show();
...
/* ===== Smart Routing+ — Group by Destination (Portal-safe, jQuery OK) ===== */
;(function(){
  // public mount
  window.cvIntelliRoutingMount = function(root){
    try {
      ...
      // --- demo data (swap for real endpoints later) ---
      ...
      // previews (cheap placeholders)
      function fetchUserPreview(){ return Promise.resolve({ topRule:"Business Hours", where:{type:"AA", name:"Main Menu"} }); }
      function fetchQueuePreview(){ return Promise.resolve({ strategy:"round-robin", timeoutSec:60, next:{ onTimeout:{type:"VM", name:"Sales VM"} } }); }
      function fetchAAPreview(){ return Promise.resolve({ keys:["1: Sales","2: Support","timeout: Main VM"] }); }
      ...
      function renderCard(g){
        ...
        if(activeDetail===g.key){
          ...
          if(g.type==='User'){ fetchUserPreview(g.id).then(function(p){ preview.innerHTML='<b>User preview:</b> Top rule <i>'+p.topRule+'</i> → '+p.where.type+' <b>'+p.where.name+'</b>'; }); }
          else if(g.type==='Queue'){ fetchQueuePreview(g.id).then(function(p){ preview.innerHTML='<b>Queue:</b> '+p.strategy+', timeout '+p.timeoutSec+'s; on timeout → '+p.next.onTimeout.type+' <b>'+p.next.onTimeout.name+'</b>'; }); }
          else if(g.type==='AA'){ fetchAAPreview(g.id).then(function(p){ preview.innerHTML='<b>AA keys:</b> '+p.keys.join(' · '); }); }
      ...
      function renderGroups(){
        ...
        var i; for(i=0;i<viewGroups.length;i++){ host.appendChild(renderCard(viewGroups[i])); }
      ...
      function applyFilters(){
        ...
        for(i=0;i<groups.length;i++){
          ...
          if(!match && /[0-9]/.test(term)){
            var j; for(j=0;j<g.numbers.length;j++){ if(g.numbers[j].number.toLowerCase().indexOf(term)>=0){ match=true; break; } }
          }
      ...
      var chips = root.querySelectorAll('.chip'); var i;
